...
class AgentLoop:
    ...
    async def _run_agent_loop(
        ...
        model: str | None = None,
    ) -> tuple[str | None, list[str], bool, list[dict]]:
        """
        ...
        Returns:
            Tuple of (final_content, tools_used, is_error, messages).
        """
        ...
        return final_content, tools_used, is_error, messages
    ...
    async def _dispatch(self, msg: InboundMessage) -> None:
        ...
        async with self._processing_lock:
            try:
                ...
                await self.bus.publish_outbound(OutboundMessage(
                    channel=msg.channel, chat_id=msg.chat_id,
                    content=_friendly_error(Exception("internal error")),
                ))
    ...
    async def _process_message(
        ...
        if msg.channel == "system":
            ...
            final_content, _, is_error, all_msgs = await self._run_agent_loop(messages)
            if not is_error:
        ...
        # on_progress is passed from process_direct() for CLI (_cli_progress).
        # For bus-based channels, _bus_progress provides edit-in-place support.
        model_override = (msg.metadata or {}).get("model")
        final_content, tools_used, is_error, all_msgs = await self._run_agent_loop(
            initial_messages, on_progress=on_progress or _bus_progress, model=model_override,
        )
        ...
        if not is_error:
            ...
            logger.warning("LLM error, not saving to session: {}", final_content[:100])
